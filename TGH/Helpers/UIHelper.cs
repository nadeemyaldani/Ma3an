    ...
    public class UIHelper
    {
        public static string GetUserTypeName(UserType userType)
        {
            if (LanguageService.IsArabic)
            {
                ...
                {
                    UserType.Public => "مستخدم",
                    UserType.Admin => "مدير نظام",
                    UserType.Approver =>"مشرف النظام",
                    _ => string.Empty
                };
            }
        ...
        {
            ResourceManager resourceManager = new ResourceManager("TGH.Resources.SharedResource", Assembly.GetExecutingAssembly());

            var test = resourceManager.GetString("SEND", CultureInfo.CurrentUICulture);
        }
